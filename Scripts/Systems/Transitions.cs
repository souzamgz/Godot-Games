...
public static class Transitions
{
    ...
    )
    {
        ...
        if (correct)
        {
            button.Scale = originalScale * 1.10f;

            await Task.Delay(120);

            button.Scale = originalScale * 0.96f;
        ...
        else
        {
            ...
            {
                float delta =
                    (float)button.GetProcessDeltaTime();
    ...
    {
        if (target == null || sourceButton == null)
        {
            return;
        }
        ...
        if (correct)
        {
            await CorrectImpact(target);
        }
        else
        {
            await WrongImpact(target);
        }
    }
    ...
    )
    {
        ...
        Label projectile = new Label();

        projectile.Text = correct
            ? "✦"
            : "·";
        ...
        while (elapsed < duration)
        {
            ...
            float delta =
                (float)target.GetProcessDeltaTime();
    ...
    )
    {
        ...
        Vector2 originalPosition = target.Position;
        Vector2 originalScale = target.Scale;
        Color originalColor = target.Modulate;

        target.Modulate = new Color("#FFF59D");
        target.Scale = originalScale * 1.14f;
        ...
        target.Scale = originalScale * 0.93f;
        target.Modulate = new Color("#FFFFFF");
        ...
        target.Scale = originalScale * 1.06f;
    ...
    )
    {
        ...
        target.Modulate = new Color("#FF7777");
    ...
    )
    {
        ...
        {
            Label fragment = new Label();

            fragment.Text = fragments[i];
        ...
        while (elapsed < duration)
        {
            ...
            {
                if (!GodotObject.IsInstanceValid(labels[i]))
                {
                    continue;
                }

                float angle =
                    (Mathf.Tau / labels.Length) * i;
